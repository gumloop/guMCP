...
class OAuthCallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        response = f"""
        <html>
        <head><title>Slack Authentication</title></head>
        <body>
...
def authenticate_and_save_credentials(user_id, service_name, scopes):
    ...
    token_response = requests.post(SLACK_OAUTH_TOKEN_URL, data=token_request_data)
    token_data = token_response.json()

    if not token_response.ok or not token_data.get("ok"):
        logger.error(
...
async def get_credentials(user_id, service_name, api_key=None):
    ...
    # For Slack, we just need the access token
