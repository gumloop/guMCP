...
async def get_credentials(user_id: str, service_name: str, api_key=None) -> dict:
    ...
    if not creds_file.exists():
        raise ValueError(f"No credentials found for user {user_id}. Please run authentication flow first.")
        
    with open(creds_file) as f:
        return json.load(f)

def authenticate_and_save_credentials(user_id: str, service_name: str, scopes: list[str]) -> None:
    ...
    try:
        ...
        redirect_uri = os.getenv("QUICKBOOKS_REDIRECT_URI", "https://developer.intuit.com/v2/OAuth2Playground/RedirectUrl")
        environment = os.getenv("QUICKBOOKS_ENVIRONMENT", "sandbox")
        
        if not all([client_id, client_secret]):
            raise ValueError("Missing required environment variables: QUICKBOOKS_CLIENT_ID, QUICKBOOKS_CLIENT_SECRET")
        ...
        print("Make sure to copy both the Authorization Code and Realm ID from the OAuth Playground page.")
        raise 