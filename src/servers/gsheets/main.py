...
def create_server(user_id, api_key=None):
    ...
    async def handle_list_tools() -> list[types.Tool]:
        return [
            ...
            types.Tool(
                ...
                inputSchema={"type": "object", "properties": {"spreadsheet_url": {"type": "string"}}, "required": ["spreadsheet_url"]},
            ),
            types.Tool(
            ...
            types.Tool(
                ...
                inputSchema={
                    ...
                    "required": ["source_spreadsheet_id", "source_sheet_id", "destination_spreadsheet_id"]
                }
            )
        ]
    ...
    async def handle_call_tool(
        ...
        if "spreadsheet_url" in arguments:
            arguments["spreadsheet_id"] = extract_spreadsheet_id(arguments["spreadsheet_url"])

        if name == "create-sheet":
            title = arguments.get("title", "New Spreadsheet")
            body = {"properties": {"title": title}}
            response = service.spreadsheets().create(body=body).execute()
            sheet_url = f"https://docs.google.com/spreadsheets/d/{response['spreadsheetId']}"
        ...
        if name == "append-values":
            ...
            return [types.TextContent(type="text", text=f"Appended {updates.get('updatedRows', '?')} rows.")]

        if name == "lookup-row":
            spreadsheet_id = arguments["spreadsheet_id"]
            values = service.spreadsheets().values().get(
                spreadsheetId=spreadsheet_id, range=arguments["range"]
            ).execute().get("values", [])
            for row in values:
        ...
        if name == "copy-sheet":
            response = service.spreadsheets().sheets().copyTo(
                ...
                body={"destinationSpreadsheetId": arguments["destination_spreadsheet_id"]}
            ).execute()
            dest_id = response.get("spreadsheetId", "")
            return [types.TextContent(type="text", text=f"Sheet copied to spreadsheet ID: {dest_id}")]
