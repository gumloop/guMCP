...
from src.utils.docusign.util import get_credentials, authenticate_and_save_credentials, process_docusign_token_response, refresh_token_if_needed
...
DOCUSIGN_BASE_URL = "https://demo.docusign.net/restapi/v2.1"  # Use production URL in production
DOCUSIGN_OAUTH_TOKEN_URL = "https://account-d.docusign.com/oauth/token"  # Demo environment
DOCUSIGN_USER_INFO_URL = "https://account-d.docusign.com/oauth/userinfo"  # For getting account information
...
async def get_account_id(docusign, account_id=None):
    ...
    for i, account in enumerate(accounts, 1):
        ...
        account_options.append(f"{i}. {account['account_name']}{is_default} - ID: {account['account_id']}")
...
def create_server(user_id: str, api_key: Optional[str] = None) -> Server:
        ...
        tools.extend([
            types.Tool(
                ...
                inputSchema={
                    "type": "object",
                    "properties": {
                        "account_id": {"type": "string", "description": "The DocuSign account ID (optional, will prompt for selection if not provided)"},
                        "search_text": {"type": "string", "description": "The search text used to search template names/subjects"},
                        "folder_ids": {"type": "string", "description": "A comma-separated list of folder IDs to filter templates"},
                        "folder_types": {"type": "string", "description": "A comma-separated list of folder types to filter templates"},
                        "include": {"type": "string", "description": "A comma-separated list of additional template data to include"},
                        "created_from_date": {"type": "string", "description": "Filter for templates created after this date (format: YYYY-MM-DD)"},
                        "created_to_date": {"type": "string", "description": "Filter for templates created before this date (format: YYYY-MM-DD)"},
                        "order_by": {"type": "string", "description": "The property to sort by (e.g., name, modified, used)"},
                        "order": {"type": "string", "description": "The sort order (asc or desc)"},
            ...
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "account_id": {"type": "string", "description": "The DocuSign account ID (optional, will prompt for selection if not provided)"},
                        "name": {"type": "string", "description": "Name of the template to create"},
                        "description": {"type": "string", "description": "Description of the template (optional)"},
                        "email_subject": {"type": "string", "description": "Default email subject for envelopes created from this template"},
                        "email_blurb": {"type": "string", "description": "Default email message for envelopes created from this template"},
                        "document_name": {"type": "string", "description": "Name of the document to include in the template"},
                        "document_base64": {"type": "string", "description": "Base64-encoded document content (PDF, DOCX, etc.)"},
                        "file_extension": {"type": "string", "description": "File extension of the document (pdf, docx, etc.)"}
                    },
                    "required": ["name", "document_name", "document_base64", "file_extension"]
                }
            ),
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "account_id": {"type": "string", "description": "The DocuSign account ID (optional, will prompt for selection if not provided)"},
                        "template_id": {"type": "string", "description": "The ID of a template to use (optional)"},
                        "email_subject": {"type": "string", "description": "Email subject line for envelope recipients (required)"},
                        "email_blurb": {"type": "string", "description": "Email message for envelope recipients"},
                        "status": {"type": "string", "description": "Envelope status: 'sent' to send immediately, 'created' to save as draft", "enum": ["sent", "created"]},
                        "recipients": {
                            "type": "array", 
                            ...
                            "items": {
                                ...
                                "properties": {
                                    ...
                                    "role_name": {"type": "string", "description": "Role name for the recipient in the template (required for template-based envelopes)"},
                                    "routing_order": {"type": "string", "description": "Order in which recipient receives the envelope (1 is first)"},
                                    "recipient_id": {"type": "string", "description": "Unique ID for this recipient (optional, will be assigned automatically if not provided)"},
                                    "tabs": {
                                        "type": "object", 
                                        ...
                                        "properties": {
                                            "signHereTabs": {
                                                ...
                                                "items": {
                                                    "type": "object",
                                                    "properties": {
                                                        "anchorString": {"type": "string", "description": "Text in the document that will be replaced with this tab"},
                                                        "anchorUnits": {"type": "string", "description": "Units for anchor positioning (pixels, inches, etc.)"},
                                                        "anchorXOffset": {"type": "string", "description": "X offset from anchor position"},
                                                        "anchorYOffset": {"type": "string", "description": "Y offset from anchor position"}
                                                    },
                                                    "required": ["anchorString"]
                                                }
                                            },
                                            "dateSignedTabs": {"type": "array", "description": "Fields that will display the date when signed"},
                                            "textTabs": {"type": "array", "description": "Text fields the recipient can fill out"}
                                        },
                        ...
                        "documents": {
                            ...
                            "items": {
                                "type": "object",
                                "properties": {
                                    "name": {"type": "string", "description": "Name/title of the document"},
                                    "document_id": {"type": "string", "description": "Document ID (must be unique within the envelope)"},
                                    "file_extension": {"type": "string", "description": "File extension (pdf, docx, etc.)"},
                                    "document_base64": {"type": "string", "description": "Base64-encoded document content"}
                                },
                                "required": ["name", "document_id", "file_extension", "document_base64"]
                            }
                        }
                    },
                    "required": ["email_subject", "status", "recipients", "documents"]
                }
            ),
            types.Tool(
                name="get_envelope",
                ...
                inputSchema={
                    ...
                    "properties": {
                        "account_id": {"type": "string", "description": "The DocuSign account ID (optional, will prompt for selection if not provided)"},
                        "envelope_id": {"type": "string", "description": "The ID of the envelope to retrieve"},
                        "include": {"type": "string", "description": "A comma-separated list of envelope parts to include (e.g., 'recipients,documents,custom_fields')"}
                    },
            ...
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "account_id": {"type": "string", "description": "The DocuSign account ID (optional, will prompt for selection if not provided)"},
                        "envelope_id": {"type": "string", "description": "The ID of the draft envelope to send"},
                        "email_subject": {"type": "string", "description": "Email subject line if not already set in the envelope"},
                        "email_blurb": {"type": "string", "description": "Email message for envelope recipients if not already set"},
                        "documents": {
                            "type": "array",
                            ...
                            "items": {
                                ...
                                "properties": {
                                    "name": {"type": "string", "description": "Name/title of the document"},
                                    "document_id": {"type": "string", "description": "Document ID (must be unique within the envelope)"},
                                    "file_extension": {"type": "string", "description": "File extension (pdf, docx, txt, etc.)"},
                                    "document_base64": {"type": "string", "description": "Base64-encoded document content"}
                                },
                                "required": ["name", "document_id", "file_extension", "document_base64"]
                            }
                        ...
                        "recipients": {
                            ...
                            "items": {
                                ...
                                "properties": {
                                    "email": {"type": "string", "description": "Recipient's email address"},
                                    "name": {"type": "string", "description": "Recipient's full name"},
                                    "recipient_id": {"type": "string", "description": "Unique ID for this recipient (optional)"},
                                    "routing_order": {"type": "string", "description": "Order in which recipient receives the envelope (1 is first)"},
                                    "tabs": {
                                        "type": "object", 
                                            ...
                                            "signHereTabs": {
                                                ...
                                                "items": {
                                                    ...
                                                    "properties": {
                                                        ...
                                                        "anchorXOffset": {"type": "string", "description": "X offset from anchor position"},
                                                        "anchorYOffset": {"type": "string", "description": "Y offset from anchor position"},
                                                        "documentId": {"type": "string", "description": "ID of the document this tab belongs to"},
                                                        "pageNumber": {"type": "string", "description": "Page number where the tab should be placed (1-based)"}
                                                    },
                                                    "required": ["anchorString"]
                                                }
                                            },
                    ...
                    "required": ["envelope_id", "email_subject", "recipients","documents"]
                }
            ),
            types.Tool(
                    ...
                    "properties": {
                        ...
                        "users": {
                            ...
                            "items": {
                                "type": "object",
                                "properties": {
                                    "userName": {"type": "string", "description": "Username for the new user (required)"},
                                    "email": {"type": "string", "description": "Email address for the new user (required)"},
                                    ...
                                    "isAdmin": {"type": "boolean", "description": "Whether the user has admin privileges"},
                                    "permissionProfileId": {"type": "string", "description": "Permission profile ID to assign to the user"},
                                    "groupId": {"type": "string", "description": "Group ID to add the user to"}
                                },
            ...
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "status": {"type": "string", "description": "Filter users by status (e.g., 'active', 'closed')"},
                        "additional_info": {"type": "string", "description": "A comma-separated list of additional user data to include"},
                        "count": {"type": "integer", "description": "Number of records to return (default is 100)"},
                        "start_position": {"type": "integer", "description": "Starting position for pagination"}
                    }
                }
            ),
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "account_id": {"type": "string", "description": "The DocuSign account ID (optional, will prompt for selection if not provided)"},
                        "user_id": {"type": "string", "description": "The ID of the user to retrieve information for"},
                        "additional_info": {"type": "string", "description": "A comma-separated list of additional user data to include (e.g., 'group_information,user_settings')"}
                    },
                    "required": ["user_id"]
                }
            ),
    ...
    async def handle_call_tool(name: str, arguments: dict | None) -> list[types.TextContent]:
        ...
        token = await get_credentials(server.user_id, SERVICE_NAME, api_key=server.api_key)
...
# Ensure arguments is a dictionary - handle string inputs
        if isinstance(arguments, str):
            try:
                ...
                return [types.TextContent(type="text", text="Error: Invalid arguments format. Arguments must be a valid JSON object.")]
        ...
        try:
            if name == "list_templates":
                ...
                templates_url = f"{docusign['base_url']}/accounts/{account_id}/templates"
                ...
                if templates_response.status_code != 200:
                    raise Exception(f"Error fetching templates: {templates_response.status_code} - {templates_response.text}")
                ...
                if template_count == 0:
                    return [types.TextContent(type="text", text="No templates found for the specified criteria.")]
                
                # Create a more readable summary
                summary = f"Found {template_count} templates in account {account_id}:\n\n"
                ...
                return [
                    ...
                    types.TextContent(type="text", text=f"Raw response:\n\n{detail_json}")
                ]
                ...
                logger.info(f"Fetching template {template_id} for account ID: {account_id}")
                ...
                if template_response.status_code != 200:
                    raise Exception(f"Error fetching template details: {template_response.status_code} - {template_response.text}")
                ...
                summary_parts.append(f"Template Name: {template.get('name', 'Unnamed Template')}")
                summary_parts.append(f"Template ID: {template.get('templateId', 'N/A')}")
                ...
                # Email settings
                summary_parts.append(f"Email Subject: {template.get('emailSubject', 'N/A')}")
                ...
                if documents:
                    ...
                    for i, doc in enumerate(documents, 1):
                        summary_parts.append(f"  {i}. {doc.get('name', 'Unnamed Document')} (ID: {doc.get('documentId', 'N/A')})")
                ...
                for field, label in recipient_types:
                    ...
                    if recipients_list:
                        ...
                        for i, recipient in enumerate(recipients_list, 1):
                            ...
                            summary_parts.append(f"    {i}. {name} ({email}) - Role: {role}, Order: {routing_order}")
                ...
                if not template_name or not document_name or not document_base64 or not file_extension:
                    return [types.TextContent(
                        type="text", 
                ...
                template_payload = {
                    ...
                    "emailSubject": arguments.get("email_subject", f"Please sign: {template_name}"),
                    "emailBlurb": arguments.get("email_blurb", "Please review and sign this document")
                }
                
                # Send the request to create the template
                template_url = f"{docusign['base_url']}/accounts/{account_id}/templates"
                
                logger.info(f"Creating template: {template_name} for account ID: {account_id}")
                ...
                if template_response.status_code not in [201, 200]:
                    raise Exception(f"Error creating template: {template_response.status_code} - {template_response.text}")
                ...
                if template_id:
                    ...
                    if recipients:
                        ...
                        for recipient in recipients:
                            ...
                            if "routing_order" in recipient:
                                template_role["routingOrder"] = recipient["routing_order"]
                    ...
                    for doc in documents:
                        if "name" not in doc or "document_id" not in doc or "file_extension" not in doc or "document_base64" not in doc:
                            return [types.TextContent(
                                type="text", 
                ...
                logger.info(f"Envelope payload: {json.dumps(envelope_payload, indent=2)}")
                ...
                if envelope_response.status_code not in [201, 200]:
                    raise Exception(f"Error creating envelope: {envelope_response.status_code} - {envelope_response.text}")
                ...
                status_message = "sent to recipients" if status == "sent" else "saved as a draft"
                ...
                if status == "sent":
                    ...
                    elif "recipients" in envelope_payload and "signers" in envelope_payload["recipients"]:
                ...
                logger.info(f"Fetching envelope {envelope_id} for account ID: {account_id}")
                ...
                if envelope_response.status_code != 200:
                    raise Exception(f"Error fetching envelope details: {envelope_response.status_code} - {envelope_response.text}")
                ...
                summary_parts.append(f"Envelope ID: {envelope.get('envelopeId', 'N/A')}")
                    ...
                    for field, label in recipient_types:
                        ...
                        if recipients_list:
                            ...
                            for i, recipient in enumerate(recipients_list, 1):
                                ...
                                routing_order = recipient.get('routingOrder', 'N/A')
                                summary_parts.append(f"    {i}. {name} ({email}) - Status: {status}, Order: {routing_order}")
                ...
                if "customFields" in envelope:
                    ...
                    if text_fields or list_fields:
                        ...
                        for field in text_fields:
                            summary_parts.append(f"  {field.get('name', 'Unnamed')}: {field.get('value', '')}")
                        
                        for field in list_fields:
                            list_items = field.get('listItems', [])
                            list_values = ", ".join(list_items) if list_items else ""
                            summary_parts.append(f"  {field.get('name', 'Unnamed')}: {list_values}")
                ...
                logger.info(f"Checking envelope {envelope_id} in account ID: {account_id} before sending")
                ...
                if envelope_response.status_code != 200:
                    raise Exception(f"Error retrieving envelope: {envelope_response.status_code} - {envelope_response.text}")
                ...
                logger.info(f"Adding {len(formatted_recipients['signers'])} recipients with tabs")
                ...
                if update_response.status_code != 200:
                    ...
                    raise Exception(f"Error updating envelope with components: {update_response.status_code} - {error_message}")
                ...
                if envelope_response.status_code != 200:
                    raise Exception(f"Error retrieving updated envelope: {envelope_response.status_code} - {envelope_response.text}")
                
                envelope = envelope_response.json()
                
                # Now try to send the envelope
                logger.info(f"Sending envelope {envelope_id} for account ID: {account_id}")
                ...
                if send_response.status_code != 200:
                    ...
                    raise Exception(f"Error sending envelope: {send_response.status_code} - {error_message}")
                ...
                success_message += f"- Documents: {len(formatted_documents)} document(s)\n"
                ...
                envelope_status_url = f"{docusign['base_url']}/accounts/{account_id}/envelopes"
                ...
                logger.info(f"Fetching status for {len(envelope_ids)} envelopes in account ID: {account_id}")
                ...
                if envelope_status_response.status_code != 200:
                    raise Exception(f"Error fetching envelope statuses: {envelope_status_response.status_code} - {envelope_status_response.text}")
                ...
                if envelope_count == 0:
                    return [types.TextContent(type="text", text="No envelope status information found for the specified IDs.")]
                ...
                for status, status_envelopes in status_groups.items():
                    ...
                    for envelope in status_envelopes:
                        ...
                        status_date = envelope.get(status_date_field, "N/A") if status_date_field else "N/A"
                        ...
                        if "recipients" in envelope:
                            ...
                            if signers:
                                ...
                                for signer in signers:
                                    ...
                                    recipient_status = signer.get("status", "Unknown status")
                                    summary += f"    - {name} ({email}): {recipient_status}\n"
                ...
                logger.info(f"Creating {len(users)} user(s) for account ID: {account_id}")
                ...
                if users_response.status_code not in [201, 200]:
                    ...
                    raise Exception(f"Error creating users: {users_response.status_code} - {error_message}")
                ...
                for i, user in enumerate(new_users, 1):
                    ...
                    if "permissionProfileName" in user:
                        summary += f"   Permission Profile: {user['permissionProfileName']}\n"
                    
                    if "userStatus" in user:
                        summary += f"   Status: {user['userStatus']}\n"
                        
                    # Check if activation email was sent
                    if "sendActivationEmail" in user and user["sendActivationEmail"] == "true":
                ...
                # Add optional query parameters if provided
                for param in ["email", "status", "username", "additional_info", "count", "start_position"]:
                    if param in arguments and arguments[param]:
                ...
                if users_response.status_code != 200:
                    raise Exception(f"Error fetching users: {users_response.status_code} - {users_response.text}")
                ...
                if user_count == 0:
                    return [types.TextContent(type="text", text="No users found for the specified criteria.")]
                ...
                user_url = f"{docusign['base_url']}/accounts/{account_id}/users/{user_id}"
                ...
                if user_response.status_code != 200:
                    raise Exception(f"Error fetching user details: {user_response.status_code} - {user_response.text}")
                
                user = user_response.json()
                # Format a human-readable summary of the user
                # summary_parts = []
                
                # # Basic user information
                # summary_parts.append(f"User ID: {user.get('userId', 'N/A')}")
                # summary_parts.append(f"Username: {user.get('userName', 'N/A')}")
                # summary_parts.append(f"Email: {user.get('email', 'N/A')}")
                # summary_parts.append(f"Status: {user.get('userStatus', 'N/A')}")
                
                # # Name information
                # name_parts = []
                # if "firstName" in user and user["firstName"]:
                #     name_parts.append(user["firstName"])
                # if "middleName" in user and user["middleName"]:
                #     name_parts.append(user["middleName"])
                # if "lastName" in user and user["lastName"]:
                #     name_parts.append(user["lastName"])
                ...
                #     summary_parts.append(f"Title: {user['title']}")
                ...
                # if "userType" in user:
                #     summary_parts.append(f"User Type: {user['userType']}")
                
                # if "isAdmin" in user:
                #     is_admin = "Yes" if user["isAdmin"] == "true" else "No"
                #     summary_parts.append(f"Administrator: {is_admin}")
                
                # if "permissionProfileName" in user:
                #     summary_parts.append(f"Permission Profile: {user['permissionProfileName']}")
                
                # # Access and settings information
                # if "createdDateTime" in user:
                #     summary_parts.append(f"Created: {user['createdDateTime']}")
                
                # if "lastLoginDateTime" in user:
                #     summary_parts.append(f"Last Login: {user['lastLoginDateTime']}")
                
                # # Group memberships if included
                ...
                #         summary_parts.append(f"  - {group_name} (ID: {group_id})")
                
                # # Account access if included
                # if "userAccountAccessRights" in user and user["userAccountAccessRights"]:
                #     access_rights = user["userAccountAccessRights"]
                #     summary_parts.append(f"\nAccount Access Rights ({len(access_rights)}):")
                #     for access in access_rights:
                #         account_name = access.get("accountName", "Unnamed Account")
                #         account_id = access.get("accountId", "N/A")
                #         access_role = access.get("accountRoleName", "Unknown Role")
                #         summary_parts.append(f"  - {account_name} (ID: {account_id}) - Role: {access_role}")
                
                # # Settings if additional info was requested
                # if "userSettings" in user and user["userSettings"]:
                #     settings = user["userSettings"]
                #     summary_parts.append(f"\nUser Settings:")
                #     for setting in settings:
                #         name = setting.get("name", "Unknown Setting")
