...
def authenticate_and_save_discourse_credentials(user_id, base_url=None):
    ...
    if not base_url:
        base_url = input("Please enter your Discourse instance URL (e.g., https://forum.example.com): ").strip()
        if not base_url:
    ...
    auth_client.save_user_credentials(
        ...
        {
            "metadata": [
                {"name": "Username", "placeholder": "Your Discourse username", "value": username},
                {"name": "Discourse URL", "placeholder": "https://example.discourse.com", "value": base_url}
            ],
            "value": api_key
        }
    )

    logger.info(f"Discourse credentials saved for user {user_id}. You can now run the server.")
    return api_key
...
async def get_discourse_credentials(user_id, api_key=None):
    ...
    if isinstance(credentials_data, dict) and "metadata" in credentials_data and "value" in credentials_data:
        metadata = {item["name"]: item["value"] for item in credentials_data.get("metadata", [])}
        return {
...
async def make_discourse_request(method, endpoint, credentials, params=None, data=None, files=None):
    ...
    try:
        async with httpx.AsyncClient() as client:
            if method.upper() == "GET":
                ...
                response = await client.post(url, headers=headers, params=params, json=data, files=files, timeout=30.0)
...
def create_server(user_id, api_key=None):
    ...
    async def handle_list_resources(
        ...
        try:
            ...
            category_list = categories_result.get("category_list", {}).get("categories", [])
        ...
        if name == "list_categories":
            try:
                ...
                return [
                    TextContent(
                        type="text",
                        text=json.dumps(categories_result, indent=2)
                    )
                ]
            ...
            try:
                ...
                return [
                    TextContent(
                        type="text", 
                        text=f"Error searching topics: {str(e)}"
                    )
                ]
            ...
            try:
                ...
                return [
                    TextContent(
                        type="text",
                        text=json.dumps(create_result, indent=2)
                    )
                ]
                ...
                return [
                    TextContent(
                        type="text", 
                        text=f"Error creating topic: {str(e)}"
                    )
                ]
            ...
            try:
                ...
                if "reply_to_post_number" in arguments and arguments["reply_to_post_number"]:
                    request_data["reply_to_post_number"] = arguments["reply_to_post_number"]
            ...
            try:
                ...
                return [
                    TextContent(
                        type="text",
                        text=json.dumps(user_result, indent=2)
                    )
                ]
