...
def mock_qb_client() -> MagicMock:
    ...
    client.session.get_base_url = MagicMock(return_value="https://quickbooks.api.intuit.com")
...
def mock_server() -> AsyncMock:
    ...
    async def mock_handle_read_resource(uri):
        ...
        if resource_type == "customers":
            return [MagicMock(text='["Amy\'s Bird Sanctuary", "Bill\'s Windsurf Shop", "Cool Cars"]')]
            ...
            return [MagicMock(text="Payment from Test Customer: $1,500.00 (2023-01-01)")]
...
@pytest.mark.asyncio
async def test_search_customers(mock_server: AsyncMock, mock_qb_client: MagicMock) -> None:
    """
...
@pytest.mark.asyncio
async def test_analyze_cash_flow(mock_server: AsyncMock, mock_qb_client: MagicMock) -> None:
    """
...
@pytest.mark.asyncio
async def test_find_duplicate_transactions(mock_server: AsyncMock, mock_qb_client: MagicMock) -> None:
    """
...
@pytest.mark.asyncio
async def test_analyze_customer_payment_patterns(mock_server: AsyncMock, mock_qb_client: MagicMock) -> None:
    """
...
@pytest.mark.asyncio
async def test_generate_financial_metrics(mock_server: AsyncMock, mock_qb_client: MagicMock) -> None:
    """
...
@pytest.mark.asyncio
async def test_error_handling(mock_server: AsyncMock, mock_qb_client: MagicMock) -> None:
    """
...
@pytest.mark.asyncio
async def test_list_resources(mock_server: AsyncMock, mock_qb_client: MagicMock) -> None:
    """
...
async def test_authentication_flow() -> None:
    ...
    """
    with patch("src.utils.quickbooks.util.authenticate_and_save_credentials") as mock_auth:
...
@pytest.mark.asyncio
async def test_customer_list_formatting(mock_server: AsyncMock, mock_qb_client: MagicMock) -> None:
    """
    ...
    with patch("src.servers.quickbooks.handlers.tools.create_quickbooks_client", return_value=mock_qb_client):
        ...
        # Test
        mock_server.handle_read_resource.return_value = [
            MagicMock(text='["Amy\'s Bird Sanctuary", "Bill\'s Windsurf Shop", "Cool Cars"]')
        ]
...
@pytest.mark.asyncio
async def test_date_range_validation(mock_server: AsyncMock, mock_qb_client: MagicMock) -> None:
    """
    ...
    with pytest.raises(ValueError, match="Invalid date format"):
        await handle_analyze_cash_flow(mock_server, {
    ...
    with pytest.raises(ValueError, match="End date cannot be in the future"):
        await handle_find_duplicate_transactions(mock_server, {
...
async def test_quickbooks() -> None:
    ...
    for invoice in invoices:
        print(f"- Invoice for {invoice.CustomerRef.name if hasattr(invoice, 'CustomerRef') else 'Unknown'}: ${invoice.TotalAmt}")
    ...
    for account in accounts:
        print(f"- {account.Name} ({account.AccountType}): ${getattr(account, 'CurrentBalance', 0)}")
    ...
    for bill in bills:
        print(f"- Bill for {bill.VendorRef.name if hasattr(bill, 'VendorRef') else 'Unknown'}: ${bill.TotalAmt}")
    ...
    for payment in payments:
        print(f"- Payment from {payment.CustomerRef.name if hasattr(payment, 'CustomerRef') else 'Unknown'}: ${payment.TotalAmt}")
    ...
    result = await handle_analyze_cash_flow(server, {
        "start_date": "2023-01-01",
        "end_date": "2023-01-31",
        "group_by": "month"
    })
    ...
    result = await handle_find_duplicate_transactions(server, {
        "start_date": "2023-01-01",
        "end_date": "2023-01-31",
        "amount_threshold": 100
    })
    ...
    result = await handle_generate_financial_metrics(server, {
        "start_date": "2023-01-01",
        "end_date": "2023-01-31",
        "metrics": ["current_ratio", "gross_margin", "net_margin"]
    })
