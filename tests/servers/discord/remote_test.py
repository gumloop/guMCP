...
def get_endpoint():
    parser = argparse.ArgumentParser(
        description="Remote MCP Test for discord server"
    )
    parser.add_argument(
        "--endpoint",
        default="http://localhost:8000/discord/local",
        help="SSE endpoint URL (default: http://localhost:8000/discord/local)",
    )
...
async def client():
    ...
    print(f"Connected to discord server at {endpoint}")
    yield client
