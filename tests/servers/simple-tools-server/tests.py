...
async def test_list_data_initial(client):
    ...
    assert "stored data" in response.lower() or "no data stored" in response.lower(), \
...
async def test_list_data_updated(client):
    ...
    assert "stored data" in response.lower(), f"Expected 'stored data' confirmation, got: {response}"
